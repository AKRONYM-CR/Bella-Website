...
function detectAndRedirect() {
  ...
  // iOS detection - abrir App Store nativo
  if (/iPad|iPhone|iPod/.test(userAgent) && !window.MSStream) {
    window.location.href = appleStoreUrl;
  }
  // macOS detection - abrir en nueva pestaña
  else if (navigator.platform.toUpperCase().indexOf("MAC") >= 0) {
    window.open(appleStoreUrl, "_blank");
  }
  // Android detection - abrir Play Store nativo
  else if (/android/i.test(userAgent)) {
    window.location.href = playStoreUrl;
  }
  // Windows/otros - abrir en nueva pestaña
  else {
